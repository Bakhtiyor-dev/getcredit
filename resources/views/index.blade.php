...
<div class="container">
  ...
  <div class="px-4  my-5 text-center border-bottom">
    ...
    <h4 class="text-muted font-monospace">Всего тестов: <span id="count">{{$count}}</span></h4>
        ...
        <form action="{{route('assesment')}}"
          ...
          <select name="subject_id" class="form-select" required>
            ...
            @foreach($subjects as $subject)
              <option value="{{$subject->id}}" @if($subject->tests_count == 0) disabled @endif>
                {{$subject->title}} ({{$subject->tests_count}})
              </option>
            @endforeach
          </select>
